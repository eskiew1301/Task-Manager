...
const Sidebar = () => {
  ...
  const NavLink = ({ element, closeSidebar }) => {
    return (
      <Link
        to={element.link}
        onClick={closeSidebar}
        className={clsx(
          "w-full lg:w-3/4 gap-2 px-3 py-2 rounded-full flex items-center text-gray-800 text-base hover:bg-[#2564ed2d]",
          path === element.link.split("/")[0] ? "bg-blue-700 text-white" : ""
        )}
      >
        {element.icon}
        <span className="hover:text-orange-500">{element.label}</span>
      </Link>
    );
  };
  return (
